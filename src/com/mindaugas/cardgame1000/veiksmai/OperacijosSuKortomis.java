...
import com.mindaugas.cardgame1000.pirminiaiduomenys.KortuSpalva;
...
public class OperacijosSuKortomis {
    ...
    public List<KortuVertes> grupuoti(List<KortuVertes> ivestis) {
        ...
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getSpalva().equals(KortuSpalva.PIKAI)) {
                stikline.add(ivestis.get(i));
            }
        }
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getSpalva().equals(KortuSpalva.KRYZIAI)) {
                stikline.add(ivestis.get(i));
            }
        }
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getSpalva().equals(KortuSpalva.BUGNAI)) {
                stikline.add(ivestis.get(i));
            }
        }
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getSpalva().equals(KortuSpalva.CYRVAI)) {
                stikline.add(ivestis.get(i));
            }
        }
...
//        }
    ...
    public List<KortuVertes> surikiuotiPagalGalia(List<KortuVertes> ivestis) {
        ...
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getVerte() ==0) {
                stikline.add(ivestis.get(i));
            }
        }

        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getVerte() ==2) {
                stikline.add(ivestis.get(i));
            }
        }

        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getVerte() ==3) {
                stikline.add(ivestis.get(i));
            }
        }

        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getVerte() ==4) {
                stikline.add(ivestis.get(i));
            }
        }
        for (int i = 0; i <ivestis.size() ; i++) {
            if (ivestis.get(i).getVerte() ==10) {
                stikline.add(ivestis.get(i));
            }
        }
...
//        }
    ...
    public List<KortuVertes> priskirtiKortosPirmam(String name, List<KortuVertes> p1, List<KortuVertes> c1, List<KortuVertes> c2) {
        ...
        if (chooseCards) {
            ...
            removes = myScan.nextInt();
            p1.remove(removes - 1);
//            Sistemoutprintas.out.println(name + " your cards are now:");
//            Sistemoutprintas.out.println(p1);
        }
    ...
    public List<KortuVertes> priskirtiKortasAntram(String name, List<KortuVertes> p2, List<KortuVertes> c1, List<KortuVertes> c2) {
        ...
        if (chooseCards) {
            ...
            p2.remove(removes - 1);
//            Sistemoutprintas.out.println(name + " your cards are now:");
//            Sistemoutprintas.out.println(p2);
        }
